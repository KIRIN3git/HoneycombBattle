...
public class PlayerMng {
    ...
    private static final String TAG = LogUtils.makeLogTag(PlayerMng.class);
...
//	static float sPlayerDpXY[][] = {{-72,106},{-82,-116},{65,150},{92,-116}};
    ...
    public static void playerInit(Context context, int numberNo, int speedNo, float sizeMagnification) {
        ...
        for (int i = 0; i < PLAYER_RADIUS_PX.length; i++)
        ...
        for (int i = 0; i < sPlayerNumber; i++) {
            if (sPlayerNumber <= 2)
                player = new PlayerStatus(i + 1, (int) (dpToPx(sPlayerDpXY2[i][0], context.getResources()) * sSizeMagnification), (int) (dpToPx(sPlayerDpXY2[i][1], context.getResources()) * sSizeMagnification), PLAYER_COLOR[i], LIFE_NUMBER);
            else
                player = new PlayerStatus(i + 1, (int) (dpToPx(sPlayerDpXY34[i][0], context.getResources()) * sSizeMagnification), (int) (dpToPx(sPlayerDpXY34[i][1], context.getResources()) * sSizeMagnification), PLAYER_COLOR[i], LIFE_NUMBER);
            players.add(player);
        }
    }
...
*/
                ...
                for (int j = 0; j < sPlayerNumber; j++) {
                    ...
                    if (i == j) continue;
                    if (PlayerMng.players.get(i).status == STATUS_DEAD || PlayerMng.players.get(j).status == STATUS_DEAD)
                        continue;
                    if (PlayerMng.players.get(i).status == STATUS_GAMEOVER || PlayerMng.players.get(j).status == STATUS_GAMEOVER)
                        continue;
...
//		paint.setAntiAlias(true);
        ...
        for (int i = 0; i < sPlayerNumber; i++) {
            ...
            for (int j = 0; j < PlayerMng.players.get(i).lifeNum - 1; j++) {
                ...
                if (i == 0 || i == 1)
                    sa_x -= ((PLAYER_RADIUS_PX[i] * 2) + dpToPx(3, context.getResources()) * sSizeMagnification);
                else
                    sa_x += ((PLAYER_RADIUS_PX[i] * 2) + dpToPx(3, context.getResources()) * sSizeMagnification);
            }
        }
...
//		PlayerMng.players.get(i).startTouchY = 0;
                        ...
                        PlayerMng.players.get(i).unrivaledTime = TimeUtils.getCurrentTime() + ItemMng.REVIVAL_UNRIVALED_TIME;
